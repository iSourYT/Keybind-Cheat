...
void grenade_prediction::Paint()
{
    ...
    if ((type) && path.size()>1)
    {
        ...
        if (Render::WorldToScreen(prev, nadeEnd))
        {
            ...
            Interfaces::Surface->DrawOutlinedCircle((int)nadeEnd.x, (int)nadeEnd.y, 5, 48);
        }
    }
}
...
void grenade_prediction::TraceHull(Vector& src, Vector& end, trace_t& tr)
{
    if (!OptionsManager.GrenadePred)
        return;
    Ray_t ray;
    ray.Init2(src, end, Vector(-2.0f, -2.0f, -2.0f), Vector(2.0f, 2.0f, 2.0f));
