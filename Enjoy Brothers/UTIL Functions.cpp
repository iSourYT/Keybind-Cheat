...
void GameUtils::CL_FixMove(CInput::CUserCmd* pCmd, Vector viewangles)
{
...
bool GameUtils::IsGrenade(void* weapon)
{
	...
	CBaseCombatWeapon *pWeapon = (CBaseCombatWeapon*)weapon;
	int id = *pWeapon->m_AttributeManager()->m_Item()->ItemDefinitionIndex();
	static const std::vector<int> v = { WEAPON_DECOY, WEAPON_HEGRENADE, WEAPON_INCGRENADE, WEAPON_MOLOTOV, WEAPON_SMOKEGRENADE, WEAPON_FLASHBANG };
	return (std::find(v.begin(), v.end(), id) != v.end());
}
...
Vector GetHitboxPosition2(int nHitbox, IClientEntity *pEntity)
{
	matrix3x4 MatrixArray[MAXSTUDIOBONES];
	...
	if (!hitbox || !pEntity->IsAlive())
		return vRet;

	if (!pEntity->SetupBones(MatrixArray, MAXSTUDIOBONES, BONE_USED_BY_HITBOX, 0/*Interfaces::GlobalVars()->curtime*/))
		return vRet;

	if (!hitbox->bone || !hitbox->bbmin.IsValid() || !hitbox->bbmax.IsValid())
		return vRet;

	VectorTransform(hitbox->bbmin, MatrixArray[hitbox->bone], vMin);
	VectorTransform(hitbox->bbmax, MatrixArray[hitbox->bone], vMax);
